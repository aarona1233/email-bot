...
// ─────────────────────────────────────────────────────────
// IMPORTANT: this client uses the SERVICE ROLE key, not the
// anon key. Service role bypasses Row Level Security by
// design — that's safe here because this file is ONLY ever
// imported from server-side code (API routes, instrumentation.js
// background jobs), never from a browser component. The browser
// never talks to Supabase directly in this app; it only calls
// our own Next.js API routes, which is where auth is actually
// enforced (see middleware.js and src/lib/supabase/server.js).
//
// SUPABASE_SERVICE_ROLE_KEY must be set in .env.local WITHOUT
// the NEXT_PUBLIC_ prefix — that prefix would expose it to the
// browser bundle, which would defeat the entire point.
// ─────────────────────────────────────────────────────────
